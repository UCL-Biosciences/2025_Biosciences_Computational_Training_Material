...
Generated by Claude 04 08 2026
...
schema so that one set of Week 1 materials works for any of them:
    ...
    sample_id | group | batch | measure_1 | <8-10 domain columns> | target
...
TARGET_AUC_SCALE = 1.6          # tuned so a simple model lands ~0.8 AUC
V2_SHIFT = 1                # how far v2 moves, in SDs; bigger = more
...
#   beta      : (a, b, scale)
#
# "signal"       columns that drive the target, with weights (for Week 5)
...
def build_frame(domain, n, version, seed):
    ...
    # Target: logistic on standardised signal columns, with noise (Week 5).
    logit = np.zeros(n)
    for name, weight in cfg["signal"].items():
        logit += weight * zscore(raw[name])
    logit = TARGET_AUC_SCALE * logit / np.sqrt(
        sum(w ** 2 for w in cfg["signal"].values())
    )
    target = (rng.random(n) < 1.0 / (1.0 + np.exp(-logit))).astype(int)
